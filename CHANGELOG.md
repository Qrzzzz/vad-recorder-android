...
- 新增多选、全选、整晚选择及批量删除确认；逐项反馈删除、收藏保留和失败数量，支持重试。
- 整晚或所选录音通过系统文件选择器导出 ZIP，保留原始 WAV，跨目录同名片段不覆盖；失败清理目标并保留原件。
- 新增收藏与删除保护：原子保存收藏状态，在仓库删除入口重新检查；兼容旧元数据，不迁移历史录音。
- 新增录音总量、收藏占用、可清理空间、每晚和所选片段容量；补齐中英文和无障碍选择控件。

## 2.5 - 2026-09-17

- #23：调整销毁和存储失败时的片段保留契约；写入前同步真实音频参数，恢复有效采样帧，保留未知残留并提供清理入口。
- 恢复固定来源文件名、跳过活跃片段，防止重复生成、覆盖正式录音和恢复已删除录音；成功恢复标记异常中断。
- #24：API 29 起观测录音配置与系统静音，界面和通知反馈限制及解除；API 26–28 受保护降级，不凭全零音频判断故障。
- 更新原销毁、清理和失败测试，并补充恢复、删除、输入状态和兼容性验收。不承诺任意强杀零损失。
